...
function getContList() {
	...
	crud.defaultAjax(url, method, data, contSuccessList, contErrorList);
}

function contSearchList(){
	...
	searchCategory = $(document).find("#contSearchCategory").val();
	searchText = $(document).find("#contSearchValue").val();
...
function contDetailView(e){
	...
	crud.defaultAjax(url, method, data, contSuccessView, contErrorView);
}

function contSuccessList(result){
	storage.soppList = result;
	window.setTimeout(drawContList, 200);
}

function contErrorList(){
	alert("에러");
}

function contSuccessView(result){
