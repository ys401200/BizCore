...
function getDetailInfo() {
  ...
  if (this.getAttribute("for").substring(0, 3) === "emp") {
    ...
    if (storage.basic !== undefined && storage.basic.no !== undefined && storage.basic.no == empNo)
...
function getHtmlBasicInfo() {
  ...
  for (x = 0; x < t.length; x++)
    html +=
      ...
      (x > 6 ? 'type="password"' : "") +
      " />" +
      (x === 5 ? '<span style="width:1.2rem;text-align:center;margin-right:0.5rem;">-</span>' : "");
  html += "</div></div>";
...
function collectEmpData(e) {
  ...
  if (t > 0 || edited)
    e.target.parentElement.parentElement.parentElement.previousElementSibling.className = "edited";
