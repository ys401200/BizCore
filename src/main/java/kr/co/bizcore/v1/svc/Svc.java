...
import kr.co.bizcore.v1.mapper.CategoryMapper;
...
import kr.co.bizcore.v1.mapper.StoreMapper;
...
public abstract class Svc {
    ...
    public int strToInt(String str) {
        int result = -1;
        try {
            result = str != null ? Integer.parseInt(str) : -1;
    ...
    public int getNextNumberFromDB(String compId, String tableName) {
        ...
        if (compId == null || tableName == null) {
            logger.debug("[Svc.getNextNumberFromDB] input data is null ::: compId : " + compId + " / tableName : "
                    + tableName);
            return result;
        }
        ...
        try {
            ...
            if (rs.next())
                result = rs.getInt(1);
            logger.debug("[Svc.getNextNumberFromDB] compId : " + compId + " / table : " + tableName
                    + " / getNextNumber : " + result);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public String cvtJsonUnicode(String str) {
        return str == null ? null
                : str.replaceAll("\"", "\\u0022").replaceAll("\r", "").replaceAll("\t", "").replaceAll("\\\\",
                        "\\u005c");
    ...
    protected long moveTempFile(String compId, String targetDir, String subDir, String savedName) {
        ...
        if (!target.exists()) {
            logger.error(
                    "Svc.moveTempFile() :::::::::: Not exist Target Directory. It Must be verify and create Web App Initializer. File Move Terminated.");
            return -2;
        }
        ...
        if (subDir != null) {
            ...
            if (!target.exists()) {
                if (!target.mkdir()) {
                    logger.error(
                            "Svc.moveTempFile() :::::::::: Request file move to sub directory under target. But, It not exist and Fail to create. File Move Terminated.");
                    return -3;
                }
            }
        }
        ...
        if (source.renameTo(target)) { // 1차 : renameTo()로 간단히 이동 시도
            ...
            try {
                ...
                source.delete();
                logger.info(
                        "Svc.moveTempFile() :::::::::: File move success. Read temp file to buffer And write target file.");
                result = target.length();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private boolean getAndProceedDeptInfo(String compId) {
        ...
        for (x = 0; x < list2.size(); x++) {
            ...
            if (dept != null)
                dept.addEmployee(userNo);
        }

        // set child
        for (x = 0; x < list1.size(); x++) {
            if (list1.get(x).equals(root))
                continue;
            parent = deptMap.get(list1.get(x).getParent());
            if (parent != null)
                parent.addChild(list1.get(x));
        }
...
class DataFactory {
    ...
    private long strToLong(String str) {
        long result = -1;
        try {
            result = str != null ? Long.parseLong(str) : -1;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
        return result;
...
class DataSet {
    ...
    public void setLifeTime(int second) {
        lifeTime = second <= 0 ? 0 : second;
    }
