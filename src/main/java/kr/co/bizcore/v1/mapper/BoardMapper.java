...
public interface BoardMapper {
    ...
    @Select("SELECT no, writer, title, IFNULL(modified, created) AS created FROM bizcore.filebox WHERE deleted IS NULL AND comp_id = #{compId} ORDER BY no")
    public List<SimpleArticle> getFileboxList(String compId);

    @Select("SELECT no, writer, title, content, created, modified FROM bizcore.filebox WHERE no = #{no} AND comp_id = #{compId} AND deleted IS NULL")
    public Article getFileboxArticle(int no, String compId);

    @Select("SELECT idx, article_no AS articleNo, ogn_name AS ognName, created, modified, size, removed FROM bizcore.filebox_attached WHERE article_no = #{articleNo} AND comp_id = #{compId}")
    ...
    @Insert("INSERT INTO bizcore.filebox(comp_id, no, writer, title, content) VALUES(#{compId}, #{article.no}, #{article.writer}, #{article.title}, #{article.content})")
