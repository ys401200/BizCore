...
public interface StoreMapper {
    ...
    @Select("SELECT * FROM swc_techstore WHERE attrib NOT LIKE 'XXX%' AND compNo = #{store.compNo} AND categories like concat('%', #{store.categories}, '%')")
    public List<Store> getCategoryStoreList(@Param("store") Store store);

    @Insert("INSERT INTO swc_techstore (storeType, productNo, compNo, contNo, custNo, userNo, locationName, firstDetail, inventoryQty, purchaseNet, serial, authCode, options, secondDetail, categories, storeDate, releaseDate, orderDate, bklnDate, regDate) VALUES (#{store.storeType}, #{store.productNo}, #{store.compNo}, #{store.contNo}, #{store.custNo}, #{store.userNo}, #{store.locationName}, #{store.firstDetail}, #{store.inventoryQty}, #{store.purchaseNet}, #{store.serial}, #{store.authCode}, #{store.options}, #{store.secondDetail}, #{store.categories}, #{store.storeDate}, #{store.releaseDate}, #{store.orderDate}, #{store.bklnDate}, now())")
