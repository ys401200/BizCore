...
public interface CommonMapper {
    ...
    @Select("SELECT desc01 AS a, value01 AS b, code01 AS c " +
            "FROM swcore.swc_code " + 
            "WHERE (code02 IS NULL OR code02 = '') AND attrib NOT LIKE 'XXX%' AND compno = (SELECT compno FROM swcore.swc_company WHERE compid = #{compId})")
    ...
    @Select("SELECT desc02 AS a, value02 AS b, code02 AS c " +
            ...
            "AND attrib NOT LIKE 'XXX%' AND compno = (SELECT compno FROM swcore.swc_company WHERE compid = #{compId})")
