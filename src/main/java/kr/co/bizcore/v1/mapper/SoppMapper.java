...
public interface SoppMapper {
    ...
    "contno AS contract, (SELECT q.custMname FROM swcore.swc_custData03 q WHERE q.custData03no = custMemberNo) AS picOfCustomer, " + 
    ...
    "AND soppno = #{soppNo} AND compno = (SELECT compno FROM swcore.swc_company WHERE compid = #{compId})")
