...
public class ApiAttachedCtrl extends Ctrl {
    ...
    public String apiAttachedSoppListGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no) {
        return sendFileList(request, "sopp", no);
    }

    @GetMapping("/contract/{no:\\d+}")
    public String apiAttachedContractListGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no) {
        return sendFileList(request, "contract", no);
    }

    @GetMapping("/docapp/{no:\\d+}")
    public String apiAttachedDocappListGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no) {
    ...
    public void apiAttachedSoppGet(HttpServletRequest request, HttpServletResponse response, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
        sendFileData(request, response, "sopp", no, fileName);
    }

    @GetMapping("/contract/{no:\\d+}/{fileName}")
    public void apiAttachedContractGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no, @PathVariable("fileName") String fileName) {
        sendFileData(request, response, "contract", no, fileName);
    }

    @GetMapping("/supplied/{no:\\d+}/{fileName}")
    public void apiAttachedSuppliedGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no, @PathVariable("fileName") String fileName) {
        sendFileData(request, response, "supplied", no, fileName);
    }

    @GetMapping("/approved/{no:\\d+}/{fileName}")
    public void apiAttachedApprovedGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no, @PathVariable("fileName") String fileName) {
        sendFileData(request, response, "approved", no, fileName);
    }

    @GetMapping("/docapp/{no:\\d+}/{fileName}")
    public void apiAttachedDocappGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no, @PathVariable("fileName") String fileName) {
        sendFileData(request, response, "appDoc", no, fileName);
    }

    // @GetMapping("/filebox/{no:\\d+}/{fileName}")
    public void apiAttachedFileboxGet(HttpServletRequest request, HttpServletResponse response,
            @PathVariable("no") int no, @PathVariable("fileName") String fileName) {
        sendFileData(request, response, "filebox", no, fileName);
    }

    private void sendFileData(HttpServletRequest request, HttpServletResponse response, String funcName, int funcNo,
            String fileName) {
        HttpSession session = null;
    ...
    public String apiAttachedSoppPost(HttpServletRequest request, @RequestBody String requestBody,
            @PathVariable int no) {
        return proceedAttachedData(request, requestBody, "sopp", no);
    }

    @PostMapping("/contract/{no:\\d+}")
    public String apiAttachedContractPost(HttpServletRequest request, @RequestBody String requestBody,
            @PathVariable int no) {
        return proceedAttachedData(request, requestBody, "contract", no);
    }

    @PostMapping("/supplied/{no:\\d+}")
    public String apiAttachedSuppliedPost(HttpServletRequest request, @RequestBody String requestBody,
            @PathVariable int no) {
        return proceedAttachedData(request, requestBody, "supplied", no);
    }

    @PostMapping("/approved/{no:\\d+}")
    public String apiAttachedApprovedPost(HttpServletRequest request, @RequestBody String requestBody,
            @PathVariable int no) {
    ...
    // @GetMapping("/filebox")
    public String apiAttachedFileboxPost(HttpServletRequest request, @RequestBody String requestBody,
            @PathVariable int no) {
    ...
    public String apiAttachedSoppPost(HttpServletRequest request, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
        return deleteAttachedFile(request, "sopp", no, fileName);
    }

    @DeleteMapping("/contract/{no:\\d+}/{fileName}")
    public String apiAttachedContractPost(HttpServletRequest request, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
        return deleteAttachedFile(request, "contract", no, fileName);
    }

    @DeleteMapping("/supplied/{no:\\d+}/{fileName}")
    public String apiAttachedsuppliedPost(HttpServletRequest request, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
        return deleteAttachedFile(request, "supplied", no, fileName);
    }

    @DeleteMapping("/approved/{no:\\d+}/{fileName}")
    public String apiAttachedapprovedPost(HttpServletRequest request, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
        return deleteAttachedFile(request, "approved", no, fileName);
    }

    @DeleteMapping("/docapp/{no:\\d+}/{fileName}")
    public String apiAttachedDocappPost(HttpServletRequest request, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
        return deleteAttachedFile(request, "appDoc", no, fileName);
    }

    // @DeleteMapping("/filebox/{no:\\d+}/{fileName}")
    public String apiAttachedFileboxPost(HttpServletRequest request, @PathVariable("no") int no,
            @PathVariable("fileName") String fileName) {
    ...
    private String deleteAttachedFile(HttpServletRequest request, String funcName, int no, String fileName) {
        ...
        if (compId == null) {
            ...
            if (v == 0) {
                ...
                result = "{\"result\":\"failure\",\"msg\":\"" + msg.fileNotFound + "\"}";
        }
        return result;
    }
